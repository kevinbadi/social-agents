...
4. Read `social-agents/social-agents.json` for `timezone` and `worker.token` (the generated `SOCIAL_AGENTS_WORKER_TOKEN`); `~/.social-agents/credentials.json` has the CreatorOS `apiKey`. Never ask about spend limits or lecture on API billing — the human knows how their credentials work.

## Procedure

Run from the workspace root:

1. `railway unlink` (a 'not linked' error is fine), then `railway init --name social-agents-worker` — ALWAYS a fresh project; never reuse an existing one.
2. Set every variable in one shot (values from the sources above — compose the command yourself, never show values in your report):
   `railway variables --set "CREATOROS_API_KEY=…" --set "<workerAiKind>=<workerAiKey>" --set "SOCIAL_AGENTS_WORKER_TOKEN=…" --set "TZ=<timezone>" --set "RAILWAY_DOCKERFILE_PATH=Dockerfile.worker"`
...
- ALWAYS a brand-new project: `railway unlink` first (ignore 'not linked' errors), then `railway init --name social-agents-worker`. NEVER link to or deploy onto any existing project — not a stale social-agents-worker from an earlier attempt, not an unrelated app, nothing. If init keeps failing, STOP and report the exact error to the human instead of retrying in a loop or hunting for an existing project to reuse.
