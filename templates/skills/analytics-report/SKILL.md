...
Read `midas/BRAND.md` (what success means for this brand), `midas/PROFILES.md`, and `midas/knowledge/COMPETITORS.md` (last known competitor state).
...
3. **Timing:** `best_time_to_post` — slots are UTC and day 0 = Monday; convert to the user's timezone from `midas/midas.json` before reporting.
4. **Competitors:** skim `midas/knowledge/COMPETITORS.md`; if stale (>2 weeks) or the human asks, refresh with web research on the handles in `midas/BRAND.md` — content mix, cadence, hooks that work, gaps to exploit — and rewrite the file.
