...
2. **Content:** `post_analytics` for the period (`fromDate`/`toDate`, `sortBy` engagement); identify the top 3 and bottom 3 posts. `daily_metrics` for the shape of the week. Look for the pattern behind the winners (hook style, format, topic) — that's the insight, not the raw numbers.
