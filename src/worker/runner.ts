...
export interface HeadlessRunOptions {
  ...
  skill: string;
  /** Names the run in the activity log (SOCIAL_AGENTS_WORKFLOW). */
  workflow: string;
...
export async function runSkillHeadless(opts: HeadlessRunOptions): Promise<RunOutcome> {
  ...
  const env: Record<string, string> = {
    ...
    SOCIAL_AGENTS_WORKFLOW: opts.workflow,
  ...
  const previousWorkflow = process.env.SOCIAL_AGENTS_WORKFLOW;
  process.env.SOCIAL_AGENTS_WORKFLOW = opts.workflow;

  const prompt =
    `Execute the "${opts.skill}" automation run now. Read social-agents/skills/${opts.skill}/SKILL.md and follow ` +
  ...
  try {
    ...
    if (previousWorkflow === undefined) delete process.env.SOCIAL_AGENTS_WORKFLOW;
    else process.env.SOCIAL_AGENTS_WORKFLOW = previousWorkflow;
  }
